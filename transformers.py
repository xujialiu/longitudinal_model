...
class TransformerEncoder(Module):
    ...
    def __init__(
        ...
        # print('USING CUSTOM TRANSFORMERENCODER')
    ...
    def forward(
        ...
        for mod in self.layers:
            ...
            # print(mod)
            # print(type(mod))
            # print(type(mod(output, src_mask=mask, is_causal=is_causal, src_key_padding_mask=src_key_padding_mask_for_layers, need_weights=need_weights)))
            # print(len(mod(output, src_mask=mask, is_causal=is_causal, src_key_padding_mask=src_key_padding_mask_for_layers, need_weights=need_weights)))
            # print(mod(output, src_mask=mask, is_causal=is_causal, src_key_padding_mask=src_key_padding_mask_for_layers, need_weights=need_weights).shape)
            # # print(mod(output, src_mask=mask, is_causal=is_causal, src_key_padding_mask=src_key_padding_mask_for_layers, need_weights=need_weights))
            output, attn_map = mod(
                output,
                src_mask=mask,
                is_causal=is_causal,
...
class TransformerEncoderLayer(Module):
    ...
    def __init__(
        ...
        # print('USING CUSTOM TRANSFORMERENCODERLAYER')
    ...
    def forward(
        ...
        # print('IN FORWARD FUNCTION OF TRANSFORMERENCODERLAYER', need_weights)

        x = src
        if self.norm_first:
            _x, attn_map = self._sa_block(
                self.norm1(x), src_mask, src_key_padding_mask, need_weights
            )
            x = x + _x
            x = x + self._ff_block(self.norm2(x))
        else:
            _x, attn_map = self._sa_block(
                x, src_mask, src_key_padding_mask, need_weights
            )
            x = self.norm1(x + _x)
            x = self.norm2(x + self._ff_block(x))

        # print('HELLO:', attn_map.shape)
    ...
    def _sa_block(
        ...
        # print('IN _sa_block:', need_weights, attn_map.shape)
